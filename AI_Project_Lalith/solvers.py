...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        for param_var, grad_tensor in zip(param_vars, grad_tensors):
            updated_param = param_var - self.learning_rate * grad_tensor
            updates.append((param_var, updated_param))
        return updates
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        for param_var, grad_tensor, vel_var in zip(param_vars, grad_tensors, vel_vars):
            velocity = self.momentum * vel_var - self.learning_rate * grad_tensor
            updated_param = param_var + velocity

            updates.append((param_var, updated_param))
            updates.append((vel_var, velocity))
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        """
        Question 6a: Optimize the model using gradient descent and return the intermediate losses.

        Optimize the model using the gradient descent algorithm by running the
        ...
        # Initialize variables shared across iterations
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        batch_train = tfu.MinibatchIndefinitelyGenerator(train_data, batch_size=1, shuffle=self.shuffle)
        batch_val = tfu.MinibatchIndefinitelyGenerator(val_data, batch_size=1, shuffle=self.shuffle)
        ...
        for iter_ in range(self.iterations):
            batch_train_data = batch_train.next()
            batch_val_data = batch_val.next()
            train_loss = session.run(loss_tensor, feed_dict={placeholders[0]: batch_train_data[0], placeholders[1]: batch_train_data[1]})
            session.run(update_ops, feed_dict={placeholders[0]: batch_train_data[0], placeholders[1]: batch_train_data[1]})
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]: batch_val_data[0], placeholders[1]: batch_val_data[1]})
