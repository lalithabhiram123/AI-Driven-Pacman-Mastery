...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        updates = []
        "*** YOUR CODE HERE ***"
        #Iterate through each parameter variable and its corresponding gradient
        for i in range(len(param_vars)):
            param_var = param_vars[i]
            gradient = grad_tensors[i]
            
            # Update the parameter using gradient descent
            new_param_tensor = param_var - self.learning_rate * gradient
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        updates = []
        "*** YOUR CODE HERE ***"
        # Iterate through each parameter variable, velocity, and gradient
        for i in range(len(param_vars)):
            velocity = vel_vars[i]
            gradient = grad_tensors[i]
    
            # Update velocity using momentum and gradient
            new_velocity = self.momentum * velocity - self.learning_rate * gradient
            updates.append((velocity, new_velocity))
    
            # Update parameter using the new velocity
            new_parameter = param_vars[i] + new_velocity
            updates.append((param_vars[i], new_parameter))
        return updates
        #util.raiseNotDefined()
