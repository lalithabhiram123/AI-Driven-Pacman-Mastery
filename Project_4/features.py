        ...
        for j in range(DIGIT_DATUM_WIDTH):
            if datum[i][j] == 0:
                if (i, j) not in visited_pixels:
                    counter += 1
                    visited_pixels+=dfs(datum, (i, j), visited_pixels)
    myFeatures = [0, 0, 0]
    if counter == 1:
        myFeatures[0] = 1
    elif counter == 2:
        ...
        myFeatures[2] = 1
    features = np.append(features, np.array(myFeatures))
    return features
...
def getNeighbors(curr, array):
    ...
    size = len(array)
    neighbors = []
    if i < size and i >= 0 and j-1 < size and j-1 >= 0:
        if (array[i][j - 1] == 0):
            neighbors.append((i, j - 1))
    if i-1 < size and i-1 >= 0 and j < size and j >= 0:
        if (array[i - 1][j] == 0):
...
def dfs(datum, state, visited_array):
    stack = util.Stack()
    stack.push(state)
    checked = []
    while not stack.isEmpty():
        curr = stack.pop()
        if curr not in checked and curr not in visited_array:
            checked.append(curr)
            neighbors = getNeighbors(curr, datum)
            for n in neighbors:
                stack.push(n)
    return checked
